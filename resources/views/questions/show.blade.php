...
<div class="bg-white rounded-xl shadow-md">
            ...
            @foreach($question->options as $index => $option)
                <div class="flex items-start p-3 sm:p-4 lg:p-4 rounded-lg border-2 {{ $option->is_correct ? 'border-green-500 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                    <div class="flex-shrink-0 mr-2 sm:mr-3">
                        ...
                        @else
                            <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 border-gray-300"></div>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm sm:text-base text-gray-900 font-medium">Option {{ $index + 1 }}</p>
                        <div class="text-sm sm:text-base text-gray-700 mt-1 prose max-w-none" id="option-content-{{ $index }}"></div>
                        @if($option->is_correct)
    ...
    <!-- Action Buttons -->
    <div class="bg-gray-50 px-4 sm:px-6 py-4 flex flex-col sm:flex-row justify-end gap-3">
        <a href="{{ route('questions.edit', $question) }}" class="w-full sm:w-auto text-center bg-indigo-600 hover:bg-indigo-700 text-white px-4 sm:px-6 py-2 rounded-lg text-sm sm:text-base font-medium transition shadow-md hover:shadow-lg">
            <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
            Edit Question
        </a>
        <form action="{{ route('questions.destroy', $question) }}" method="POST" class="w-full sm:w-auto" onsubmit="return confirm('Are you sure you want to delete this question?');">
            @csrf
            @method('DELETE')
