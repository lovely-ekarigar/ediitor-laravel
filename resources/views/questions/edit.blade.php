        ...
        <div class="mb-6">
            ...
            <div id="optionsContainer" class="space-y-3">
                @foreach($question->options as $index => $option)
                    <div class="option-row flex flex-col sm:flex-row items-start gap-3 p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-200" data-option-index="{{ $index }}">
                        ...
                        <div class="flex-1 w-full">
                            ...
                            <div id="editorjs-option-{{ $index }}" class="border border-gray-300 rounded-lg bg-white min-h-[200px] p-2 sm:p-3 @error('options.'.$index.'.option_text') border-red-500 @enderror" style="cursor: text;"></div>
                            <input type="hidden" name="options[{{ $index }}][option_text]" id="option_text_{{ $index }}" value="{{ old('options.'.$index.'.option_text', $option->option_text) }}">
                        </div>
                        <button 
                            type="button" 
...
<script>
    ...
    // Function to detect if content is JSON or HTML
    function parseQuestionContent(content) {
        ...
        // If HTML, convert to Editor.js format
        // Strip HTML tags and create a paragraph block
    ...
    waitForEditorJS(function() {
        ...
        const existingContent = hiddenInput ? hiddenInput.value : null;
        const editorData = parseQuestionContent(existingContent);
    ...
    waitForEditorJS(function() {
        // Get existing option content and initialize editors
        const optionRows = document.querySelectorAll('.option-row');
        optionRows.forEach((row, i) => {
            const index = parseInt(row.getAttribute('data-option-index'));
            const hiddenInput = document.getElementById(`option_text_${index}`);
            const existingContent = hiddenInput ? hiddenInput.value : null;
            const editorData = parseOptionContent(existingContent);
            
            setTimeout(() => initOptionEditor(index, editorData), i * 100);
        });
    });
